...
function get_fb_friends($param){
	...
	$sql_insert = "INSERT INTO {$param->table_name}(facebook_id,friends_fbid,friends_name,friend_location_name,created_date) VALUES ";
	$sql = '';
	foreach($fbdata as $data){			
		...
		if (mysql_num_rows($result)==0){
			...
			$fb = @json_decode(file_get_contents($graph_url_of_friend));
			print_r($fb);
			echo '===============================';
