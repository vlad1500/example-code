...
// internal constants
    ...
    HASH = window.location.hash.replace("#Slideshow~", ''),
...
$.event.special['click:fast'] = {
    ...
    add: function(handleObj) {
        ...
        if ( Galleria.TOUCH ) {
            ...
            $(this).live('click', handleObj.handler);
        }
    },
    ...
    _createThumbnails : function( push ) {
        ...
        for( ; this._data[ i ]; i++ ) {
            ...
            if ( ( o.thumbnails === true || optval == 'lazy' ) && ( data.thumb || data.image ) ) {
                ...
                thumb.video = false;
                //if(iCount == 0){
//                    var thumbCon = $(document.createElement('div'))
//                        .addClass('thumb_con')
//                        .addClass('MenuItem')
//                        .attr("id","Slideshow-MenuItem-"+conCount)
//                        .appendTo($( '.galleryThumbCon' ))
//                    ;
//                }
                // append the thumbnail
                $( '.galleryThumbCon' ).append( thumb.container );
                //iCount++;
//                if(iCount == 10){
//                    conCount++;
//                    iCount = 0;
//                }
                ...
                //josh
                var imgCount = 11;
                var winSize = this._getWH();
                var conWidth = (winSize.width / 3)-100;
                var tW = conWidth / ((imgCount*1)+1);
                var tH = (tW / 11) * 8;
                $container.attr("id","Slideshow-MenuItem-"+i);
                $container.addClass("MenuItem");
                //console.log("imgCount: "+imgCount+" | winSize: "+winSize+" | conWidth: "+conWidth+" | twidth: "+tW+" | tHeight: "+tH);
                //end josh
                // hide it
                $container.css( 'visibility', 'hidden' );
                $container.css( { display: 'table-cell',"vertical-align":"middle" } );
                thumb.data = {
                    width  : Utils.parseValue( tW+"px" ),
                    height : Utils.parseValue( tH+"px" ),
                    order  : i,
                    src    : src
                };
                ...
                if ( o.thumbCrop !== true ) {
                    ...
                    $container.css( { width: tW+"px", height: tH+"px" } );
                }
    ...
    */
    ...
    lazyLoad: function( index, complete ) {
        ...
        $.each( arr, function(i, ind) {
            ...
            //console.log($( thumb.container ));
            if ( thumb.video ) {
                thumbload.call( self, thumb, callback );
            } else {
                thumb.load( data.src , function( thumb ) {
                    thumbload.call( self, thumb, callback );
                    var thisSrc = $('.MenuItem').find('img[src$="'+data.src+'"]');
                    //console.log(thisSrc);
                    if(thisSrc.length > 1){
                        var thisCount = thisSrc.length;
                        var curCount = 1;
                        thisSrc.each(function(){
                            var imgCount = 11;
                            var winSize = self._getWH();
                            var conWidth = (winSize.width / 3)-100;
                            var tW = conWidth / ((imgCount*1)+1);
                            var tH = (tW / 11) * 8;
                            ...
                            if(curCount != thisCount)
                                ...
                                $(this).attr("width",tW);
                                $(this).parent().css("width",tW+"px");
                                $(this).css("height",tH+"px");
                                $(this).attr("height",tH);
                                $(this).parent().css("height",tH+"px");
                                //$(this).hide();
                            }
                            curCount++;
                        });
...
this.prependChild( 'info', 'myElement' );
    ...
    */
    ...
    _show : function() {
        ...
        //console.log("this is data.image:");
        //console.log(data.image);
        //console.log(this._options.dataSourceOrig);
        var src = this.isFullscreen() && data.big ? data.big : ( data.image || data.iframe ),
        ...
        if(src && src.indexOf("/images/loader_slide.gif") != -1){
            var curIndex = queue.index - 2;
            var pTotal = this._data.length;
            var range = self.getRange(curIndex,pTotal);
            var setNum = range/10;
            //console.log(curIndex+" ~ "+setNum+" ~ "+pTotal);
            var thisImage = this._options.dataSourceOrig[setNum][curIndex].image;
            //console.log(curIndex+" ~ "+thisImage);
        }
    ...
    scale: function( options ) {
        ...
        Utils.wait({
            ...
            success: function() {
                ...
                // apply position
                var imgUrlThis = self.image.src;
                if(imgUrlThis.indexOf("150x150") == -1){
                $( self.image ).css({
                    position : 'absolute',
