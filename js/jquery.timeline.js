...
//			});
			...
			$this.find(settings.openTriggerClass).on("click",function(){
                ...
                setTimeout(function () {
                    ...
                    $this.timeline('goTo',self.attr('data-id'), self.attr('data-count'), true);    
                }, 1000);				
			});
		...
		open : function (id, data_count) {
			...
			$items.each(function(){			     
				if ($(this).attr('data-id') == id) {
				    console.log("opening more: "+id+": this id: "+$(this).attr('data-id'));
				    if($(".nTimeImg").length > 0)
                        $(".nTimeImg").each(function(){
				            $(this).remove();    
				        });
				    var thisImg = $(this).find("img");
                    var nHeight = (width / 11) * 8;
                    var newSrc = '/timthumb.php?src='+thisImg.attr("originalSrc")+'&h='+nHeight+'&w='+width+'&zc=2';                    
                    thisImg.hide();
                    var newImg = $(document.createElement('img'));
                    newImg.attr("src",newSrc);
                    newImg.addClass("nTimeImg");
                    newImg.insertAfter(thisImg);
...
'	 </div>\n';
			...
			$items.each(function(){
                var dataIDed = ($(this).attr('data-id'));
                var today = new Date();
                var dd = today.getDate();
                var mm = today.getMonth()+1;
                var yyyy = today.getFullYear();
                if(typeof dataIDed === 'undefined') dataIDed = dd+"/"+mm+"/"+yyyy+" 0:0:0";
                if(dataIDed == "undefined") dataIDed = dd+"/"+mm+"/"+yyyy+" 0:0:0";
                if(dataIDed == "") dataIDed = dd+"/"+mm+"/"+yyyy+" 0:0:0";
                //console.log(dataIDed);
				var dataId = dataIDed,
                ...
                var dVal = (d/(data.options.categories ? monthsDays[m] : monthsDays[1]))*100;
                var vPer = 100/(data.options.categories ? monthsDays[m] : monthsDays[1]);
                var vPH = vPer/24;
                var vPM = vPH/60;
                var vPS = vPM/60;
                var uVal = dVal-vPer;
                //console.log("start val: "+dataId);
			    if(hrs != 0)uVal = uVal+(vPH*hrs);
                //console.log("hrs: "+uVal);
                if(min != 0)uVal = uVal+(vPM*min);
                //console.log("min: "+uVal);
                if(sec != 0)uVal = uVal+(vPS*sec);
                //console.log("sec: "+uVal);                
				var curLeft = uVal;                
                //console.log("total: "+curLeft);                		
				// Store node element
				nodes[dataId] = '<a href="#'+dataId+'" class="t_line_node'+(cnt == data.currentIndex ? ' active' : '')+'" style="left: '+curLeft+'%; position:absolute; text-align:center;">'+((typeof nodeName != 'undefined') ? nodeName : d);
