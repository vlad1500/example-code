...
var _arr_;
	$(function(){
		...
		//Book_Summary Page Scripts -------------------------------------------------------------------------------------------------
		/*$('#album_for_me').live('click',function(){
			$('#app_loader').fadeIn('slow');
			$.ajax({
			...
			$.ajax({
				...
				success : function(res){
					...
					$("ul.tabs2 li").removeClass("active"); //Remove any "active" class
					$('ul.tabs2 li#data').addClass("active").fadeIn(); //Add "active" class to selected tab					
					$('#main_inner').html(_obj.data).fadeIn('slow',function(){
						$('#fb_data').fadeIn('slow',function(){
							loadAlbum();
							$('#app_loader').fadeOut('slow');
						});									
					});					
 ...
 //$('#main_inner_uploder_pop_a').click();
		...
		$("#album_for_me_unique").on("click", function() {
		    console.log("album for me clicked");
			$('#shared_active').css('display','block');
		//	$("#app_loader").fadeIn("slow");
			$.ajax({
		...
		$("#album_create_name").live("click", function() {
			$('#book_name_pop').val('');
			$('.err_div').html(''); 
						...
						'success'	: function(res){
							...
							ajax.upload.addEventListener('progress',function(evt){
								...
								if (evt.lengthComputable) {
									...
									$('#progressnum').val(Math.round(evt.loaded / evt.total * 100) + "%");
	...
	/*(function($) {
		$.fn.myModal = function (settings) {
			var opts =  $.extend({},$.fn.myModal.defaults, settings);
			var _getobj;
			
			this.each(function () {
				var html = '<div id="modal_container">';
					html += '<a href="#" id="cancel" class="float_right"><img src="../images/close.png" /></a>';
				    html += '<div id="modal_inner" style="overflow:hidden"></div></div>';
					html += '<div id="modal_bckgrnd">';
					html += '';
					html += '</div>';
					html += '';
				$('body').prepend(html);
				...
				$('#modal_bckgrnd').css({'opacity':0}).animate({opacity:.7},'slow',function(){
					$(this).append('<div class="ajax_loader"></div>');
					$('#modal_container').css({'width':$('body').width(),'height':$('body').height(),'opacity':1}).fadeIn('slow',function(){ $('.ajax_loader').remove(); });
					if (opts.inline == true){
						inline();
					}else if(opts.sendMessage == true){
						sendMessage();
					}else if(opts.friendSelector == true){
						friendSelector();
					}else if(opts.deletePages == true){
						deletePages();
					}else if(opts.saveAs == true){
						save_as();
					}else if(opts.saveAs == true){
						AddFriendsToBook_msg();
					}else if(opts.printBook == true){
						printBook();
					}
				});//end of $('#modal_bkgrnd')
	...
	function deletePages () {
		...
		$('#pages').scroll(function(){
			if (mincount <= arrObj.length){				
				if($(this).scrollTop() + $(this).height() >= $(this).height() - 400) {
					$("#pages li").slice(mincount,maxcount).fadeIn(800);
	...
	function post_to_timeline(_userid){
		...
		/*
		$.ajax({
			url		: 'http://www.facebook.com/dialog/apprequests?app_id=331059976950036&message='+_msg+'&redirect_uri=http://hardcover.shoppingthing.com/main/share_url?book_info_id='+_infoid,
			type	: 'post',
			error	: function(res){
						console.log(res);
					},
	...
	/*
	var params = {
		_url		: gSiteUrl + "ajax/savefbbutton/",
		postParam	: "store_id=" + storeId  + "&mode=" + mode + "&page_id=" + pageId,
		onSuccessFn	: function( result ){
			if( result.error == 0 ){
				ajaxMiddle = false;
				setTimeout("showTab('mypages', '1');$('#createStoreMainTab').addClass('currentlinksimport');", 300 );
			} else {
		...
		*/
		...
		$('section').each(function(index, element) {
			$(this).on('mousedown',function(){
				...
				//$(document).off('mousedown',mouseDownHandler).off( "mouseup", mouseUpHandler);
			});
		});
		*/
		var _lastPage = 0;
...
// jQuery(this).parents('iframe').setAttribute("scrolling","no");
//});
