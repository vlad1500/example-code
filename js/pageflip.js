...
(function() {
	...
	function mouseDownHandler( event ) {
		if (Math.abs(mouse.x) < PAGE_WIDTH) {
			if (mouse.x < 0 && page - 1 >= 0) {
				flipsE[page - 1].dragging = true;
			}
			else if (mouse.x > 0 && page + 1 < flipsE.length) {
				flipsE[page].dragging = true;
			}
		}
	...
	function mouseUpHandler( event ) {
		for( var i = 0; i < flipsE.length; i++ ) {
			// If this flip was being dragged we animate to its destination
			if( flipsE[i].dragging ) {
				// Figure out which page we should go to next depending on the flip direction
				if( mouse.x < 0 ) {
					flipsE[i].target = -1;
					page = Math.min( page + 1, flipsE.length );
				}
				else {
					flipsE[i].target = 1;
					page = Math.max( page - 1, 0 );
